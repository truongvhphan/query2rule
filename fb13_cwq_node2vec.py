...
embedding_ent384 = 'data/FB13/entity2vec512.unif'
...
fb_question_rules = 'data/CWQ_question_rules.csv'
cwq_question_ner = 'data/fb_CWQ_question.csv'
max_length = 384
...
with open(cwq_question_ner, 'r', encoding='utf-8') as f:
    data = csv.reader(f, delimiter='\t')
    for d in data:
        question_ner.append([d[0], d[2]])

embeddings = np.loadtxt(embedding_boxe_ent384, delimiter=' ')
# embeddings = np.loadtxt('data/FB13/entity2vec256.unif', delimiter=' ')
    ...
    for r in rules:
        try:
            ...
            if str(h).replace(' ','_').lower() in ent2id.keys() and str(h).replace(' ','_').lower() not in entities:
                embed_entity.append(embeddings[ent2id[str(h).replace(' ','_').lower()]][:max_length])
                entities.append(str(h).replace(' ','_').lower())
            if str(t).replace(' ','_').lower() in ent2id.keys() and str(t).replace(' ','_').lower() not in entities:
                embed_entity.append(embeddings[ent2id[str(t).replace(' ','_').lower()]][:max_length])
...
# #
# with open('data/FB13/kdtree_fb_vector.pkl', 'wb') as kg:
...
llm = 'bert'
ds = 'cwq'
f = open(f'data/FB13/results384_{ds}_{kgmodel}_{llm}.csv', 'w', encoding='utf-8', newline='')
    ...
    if ques != None:
        ...
        for distance, index in zip(distances[1:], indices[1:]):
            ...
            if id2ent[index] not in types:
                ...
                tem_ent.append(embeddings[index][:max_length])
                # print(ent_in_rule)
                text = ''
                if id2ent[index] in ent2text.keys():
                    ...
                    if check_entity_in_text(all_ents, text) == True:
                        ...
                        print(f'{ques[0]} -- {str.lower(ques[1])} -- {id2ent[index]} -- {rs}')
                        if str.lower(ques[1]) == id2ent[index]:
                            answer_ent = id2ent[index]
                            f_distance = distance
                            break
                        else:
        ...
        if str.lower(ques[1]) == answer_ent:
            print(f'Question: {i}/{len(question_rules)} -- Answer: {ques[1]} -- {answer_ent} -- True -- Time: {e-s}  -- {j} -- {f_distance}')
            w.writerow([i, len(indices[1:]), str.lower(ques[1]), ent2type[str.lower(ques[1])], answer_ent, ent2type[answer_ent] ,  True, e - s, j, f_distance])
        else:
            if answer_ent != '' and str.lower(ques[1]) in ent2id.keys():
                print(f'Question: {i}/{len(question_rules)} -- Answer: {ques[1]} -- {answer_ent} -- False -- Time: {e-s}')
                w.writerow([i,len(indices[1:]), str.lower(ques[1]), ent2type[str.lower(ques[1])], answer_ent, ent2type[answer_ent], False, e - s, j , f_distance])
f.close()
te = time.time()
